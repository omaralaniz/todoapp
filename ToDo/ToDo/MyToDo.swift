...
extension Collection where Iterator.Element == ToDoItem {
    ...
    func writeToPersistence() throws{
        if let url = Self.persistencePath(), let array = self as? NSArray{
            ...
            throw NSError(domain: "com.oalaniz.ToDo", code: 10, userInfo: nil)
        }
    }
    static func readFromPersistence() throws -> [ToDoItem]{
        if let url = persistencePath(), let data = (try Data(contentsOf: url) as Data?){
            if let array = NSKeyedUnarchiver.unarchiveObject(with: data) as? [ToDoItem]{
                ...
                throw NSError(domain: "com.oalaniz.ToDo", code: 11, userInfo: nil)
            }
        } else {
            throw NSError(domain: "com.oalaniz.ToDo", code: 12, userInfo: nil)
        }
    }
}
